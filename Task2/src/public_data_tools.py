...
def _get_json_with_retry(url: str, params: dict, timeout: int = 15) -> dict:
    """Fetch JSON reliably on macOS; fall back to curl after transient TLS errors."""
    headers = {"User-Agent": "ai-agent-book-task2/1.0"}
    ...
    for attempt in range(3):
        try:
            ...
            last_error = exc
            logging.warning("HTTP attempt %s/3 failed: %s", attempt + 1, exc)
            time.sleep(attempt + 1)

    # Some macOS/Python combinations intermittently abort TLS reads while the
    # system curl client succeeds. Keep this fallback inside the Tool so the
    # MCP/Agent flow remains unchanged.
    prepared_url = requests.Request("GET", url, params=params).prepare().url
    try:
        completed = subprocess.run(
            ["curl", "--fail", "--silent", "--show-error", "--retry", "3",
             "--retry-all-errors", "--max-time", str(timeout), prepared_url],
            check=True,
            capture_output=True,
            text=True,
        )
        ...
        raise RuntimeError(f"HTTP request failed after retries: {last_error}; curl fallback: {exc}") from exc
...
async def get_weather(
    ...
    try:
        ...
        if latitude is None or longitude is None:
            ...
            geocode_data = _get_json_with_retry(geocode_url, geocode_params)
        ...
        data = _get_json_with_retry(weather_url, weather_params)
